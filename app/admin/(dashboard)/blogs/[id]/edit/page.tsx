...
export default function EditBlogPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    params.then(({ id: blogId }) => {
      setId(blogId);
      supabase.from('blogs').select('*').eq('id', blogId).single().then(({ data }) => {
        if (data) {
          setTitle(data.title); setSlug(data.slug); setExcerpt(data.excerpt);
          setContent(data.content); setTags(data.tags?.join(', ') ?? '');
          setReadTime(data.read_time ?? '5 min read'); setStatus(data.status);
          if (data.cover_url) setCoverPreview(data.cover_url);
          if (data.cover_storage_path) setExistingCoverPath(data.cover_storage_path);
        }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    let cover_url = coverPreview.startsWith('blob:') ? null : coverPreview;
    let cover_storage_path = existingCoverPath;

    if (coverFile) {
      // Remove old cover
      if (existingCoverPath) await supabase.storage.from('blog-covers').remove([existingCoverPath]);
    ...
    const { error: dbErr } = await supabase.from('blogs').update({
  ...
  if (fetchLoading) return <div style={{ padding: '60px', textAlign: 'center', color: '#888' }}>Loading...</div>;
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: '24px', alignItems: 'start' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            <Card>
              ...
              />
              <Label style={{ marginTop: '14px' }}>Slug</Label>
              <input value={slug} onChange={(e) => setSlug(slugify(e.target.value))}
                style={{ ...INPUT_STYLE, fontFamily: 'monospace', fontSize: '0.82rem' }}
                onFocus={(e) => { e.currentTarget.style.borderColor = '#ffcc45'; }}
                onBlur={(e) => { e.currentTarget.style.borderColor = 'rgba(67,36,48,0.18)'; }}
              />
            </Card>
            ...
            <Card>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
                <Label style={{ margin: 0 }}>Content <span style={{ color: '#aaa', fontWeight: 400 }}>(Markdown)</span></Label>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            <Card>
              ...
              <div style={{ display: 'flex', gap: '8px' }}>
                {(['draft', 'published'] as const).map((s) => (
                  <button key={s} type="button" onClick={() => setStatus(s)} style={{
                    ...
                    fontWeight: 700, fontSize: '0.8rem', border: 'none', transition: 'all 0.2s', textTransform: 'capitalize',
              ...
              {error && <div style={{ marginTop: '12px', padding: '10px', borderRadius: '8px', background: 'rgba(220,38,38,0.06)', color: '#dc2626', fontSize: '0.8rem' }}>{error}</div>}
            ...
            <Card>
              ...
              <label style={{
                ...
                border: '2px dashed rgba(67,36,48,0.2)', color: '#666',
              }}>
            ...
            <Card>
              ...
              <input value={tags} onChange={(e) => setTags(e.target.value)} placeholder="Velvet, Care Tips"
                style={INPUT_STYLE}
                onFocus={(e) => { e.currentTarget.style.borderColor = '#ffcc45'; }}
                onBlur={(e) => { e.currentTarget.style.borderColor = 'rgba(67,36,48,0.18)'; }}
              />
...
function Card({ children }: { children: React.ReactNode }) {
  return <div style={{ background: '#fff', borderRadius: '12px', padding: '22px', border: '1px solid rgba(67,36,48,0.08)' }}>{children}</div>;
}

function Label({ children, style }: { children: React.ReactNode; style?: React.CSSProperties }) {
  return <label style={{ display: 'block', fontSize: '0.75rem', fontWeight: 700, color: '#432430', letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: '8px', ...style }}>{children}</label>;
}
